...
# from https://github.com/chrisdickinson/python-javascript/blob/master/javascript/tokenizer.py
...
def tokenize(stream):
    ...
    str = []
    numrange = list(range(ord('0'), ord('9')+1))
    octrange = list(range(ord('0'), ord('7')+1))
    hexrange = list(range(ord('a'), ord('f')+1)) + list(range(ord('A'), ord('F')+1)) + numrange

    whitespace = [' ', '\n', '\t', '\r', '"', "'"]
    get_operators_for = lambda c : [x for x in operators if ord(c) is ord(x[0])]
        ...
        if in_comment:
            ...
            if escaped:
                ...
                if character == '\\':
                    ...
                    in_string = False
                    tokens.append(Token(''.join(str), Token.String, line, char))
                    str = []
                else:
                    str.append(str(character))
            ...
            if not building_name and not is_sep:
                if is_number(character):
                    ...
                    if character == '0':
                        if next() == 'x':
                            ...
                            while is_hexnumber(c):
                                ...
                                c = next(skip)
                            skip -= 1
                            tokens.append(Token(''.join(str), Token.Number, line, char))
                            str = []
                            continue
                    ...
                    skip = 0
                    okay = lambda : is_number(next(skip)) or (not seen_e and next(skip) == '.') or (not seen_dot and next(skip) is not None and next(skip).lower() == 'e')
                    while okay(): 
                        c = next(skip) 
                        str.append(c)
                        if c == '.':
                ...
                if character == '/' and tokens[-1].type == Token.Operator:
                    ...
                    if not in_regex:
                        ...
                        str.append('/')
                        str = ['/'] + str + flags
                        tokens.append(Token(''.join(str), Token.Regex, line, char)) 
                        str = []
                        continue
